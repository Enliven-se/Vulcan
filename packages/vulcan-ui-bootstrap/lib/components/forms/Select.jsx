...
const SelectComponent = ({
  ...
  const noneOption = {
    ...
    value: getFieldType(datatype) === String || getFieldType(datatype) === Number
      ? ''
      : null, // depending on field type, empty value can be '' or null
    disabled: true
  };
  let otherOptions = Array.isArray(inputProperties.options) && inputProperties.options.length
    ? inputProperties.options
    : [];
  const options = [
    noneOption, ...otherOptions
  ];
  return (
