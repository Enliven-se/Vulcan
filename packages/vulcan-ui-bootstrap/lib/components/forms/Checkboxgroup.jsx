...
const CheckboxGroupComponent = ({
  ...
  // get rid of duplicate values or any values that are not included in the
  // options provided
  value = uniq(intersection(value, options.map(o => o.value)));

  // if this is a "new document" form check options' "checked" property to
  // populate value
  if (formType === 'new' && value.length === 0) {
    const checkedValues = _
      .where(options, {checked: true})
  ...
  const {
    onChange,
    ...newInputProperties
  } = inputProperties; // eslint-disable-line no-unused-vars

  return (
    <Components.FormItem {...newInputProperties} {...itemProperties}>
      <div>
        {options.map((option, i) => (<Form.Check
          layout="elementOnly"
          key={i}
          {...newInputProperties}
          label={option.label}
