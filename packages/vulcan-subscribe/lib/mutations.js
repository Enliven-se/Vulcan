...
const subscribeMutationsGenerator = collection => {
  ...
  const genericMutationFunction = (collectionName, action) => {
    ...
    return function (root, {
      ...
      // permission check if (!Users.canDo(context.currentUser,
      // `${collectionName}.${action}`)) {   throw new Error(Utils.encodeIntlError({
      // id: 'app.noPermission' })); } do the actual subscription action
